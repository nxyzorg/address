...
| 既有门牌级格式 / 邮编 | `<street> <house>, <postcode> <locality>, DE`；Wohnung/Etage 缺失保持空；OpenPLZ/源记录；格式门禁 5 位数字；不得用几何最近邮编补齐。只清洗格式，不创造或按邻近地址补齐 |
| 行政区 | Destatis GV100AD 作为候选核验；OpenPLZ 街道目录仅辅助，不覆盖门牌。行政层级必须与坐标反查一致；冲突时保留源值在隔离区并拒绝发布 |
| 住宅子集证据 | OSM 明确住宅建筑标签；街道目录或邮编多边形不构成住宅证据。住宅证据必须来自明确建筑/用途字段，不能由地址存在推断 |
...
| 街道级格式 | `<street>, <municipality>, DE`；`matchLevel=street`，门牌、楼栋、室号为空；可唯一确定的来源邮编保留，否则留空。 |
...
| 既有门牌来源验证 / 排除 | 缺有效城市层级、真实道路或合法坐标的记录淘汰；未知 PLZ 留空，住宅建筑证据仅用于住宅子集；OpenPLZ 仅作离线辅助。 |
| 策略版本 / 状态 / 更新 | 1.10 / 16 州分片严格同步 + 多规则完成下限 + 街道契约 street-v1 / 2026-09-13 |

- 默认国家完成数量下限 40,000；州、市镇、区单来源均衡采样上限 2,500/350/80，后台可覆盖；国家总量不是发布上限，行政区划与邮编目录保持完整。
