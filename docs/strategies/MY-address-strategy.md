...
| 既有门牌级格式 / 邮编 | `<unit/lot>, <building>, <street>, <postcode> <town>, <state>, MY`；缺失 unit 不补；源记录；格式门禁 5 位数字。只清洗格式，不创造或按邻近地址补齐 |
| 行政区 | 源字段与 catalog；DOSM 行政目录作为候选核验。行政层级必须与坐标反查一致；冲突时保留源值在隔离区并拒绝发布 |
| 住宅子集证据 | OSM 明确住宅建筑标签；商业 POI 黑名单剔除。住宅证据必须来自明确建筑/用途字段，不能由地址存在推断 |
...
| 街道级格式 | `<street>, <town>, <state>, MY`；`matchLevel=street`，门牌、楼栋、室号为空；可唯一确定的来源邮编保留，否则留空。 |
| 街道去重 / 验证 | 国家 + 规范行政层级 + 街道名，同一路段不同坐标、邮编或来源 ID 不增加条数；行政字段不使用最近目录点推断。 |
| 同步方式 | 自动初始化一次；完整扫描或来源耗尽后停止。显式来源探测发现 Geofabrik 版本变化时才构建新快照 |
| 既有门牌来源验证 / 排除 | 同一 PBF 按 MYS/SGP 边界分片。缺真实街道、必填行政字段或坐标时不发布；门牌缺失标为 street，未知邮编留空。TEDUH 名称本身不能替代地址或住宅证据。 |
| 策略版本 / 状态 / 更新 | 1.11 / 严格住宅点面关联，TEDUH 候选核验 + 多规则完成下限 + 街道契约 street-v1 / 2026-09-13 |

- 默认国家完成数量下限 10,000；州/联邦直辖区、县市、地区单来源均衡采样上限 1,500/250/60，后台可覆盖；国家总量不是发布上限，行政区划与邮编目录保持完整。
