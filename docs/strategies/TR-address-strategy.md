...
| 既有门牌级格式 / 邮编 | `<street> <no>, <district>/<province> <postcode>, TR`；源记录；格式门禁 5 位数字。只清洗格式，不创造或按邻近地址补齐 |
| 行政区 | 源字段与 catalog；TurkStat 行政目录作为候选核验。行政层级必须与坐标反查一致；冲突时保留源值在隔离区并拒绝发布 |
| 住宅子集证据 | OSM 住宅建筑标签；Building Identity 的 `ZEMIN_KAT/UST_KAT=Konut` 可证明建筑住宅用途 |
| 发布门禁 | 接受真实街道级与已有门牌级地址；按实际精度检查来源、行政归属、语言和坐标。街道记录不强制门牌，不编造缺失字段；已有真实门牌保留，未知或歧义邮编留空；只有住宅子集要求独立住宅证据。 |
| 街道级格式 | `<street>, <neighborhood>, <district>, <province>, TR`；`matchLevel=street`，门牌、楼栋、室号为空；可唯一确定的来源邮编保留，否则留空。 |
...
| 既有门牌来源验证 / 排除 | 源缺坐标或邮编时，完成地块/OSM 精确关联和邮编唯一匹配前不发布。NVI MAKS/UAVT、e-Devlet、ATLAS、TKGM 不作为匿名批量源。 |
| 策略版本 / 状态 / 更新 | 1.12 / 严格住宅关联 + 多规则完成下限 + 街道契约 street-v1 / 2026-09-13 |

- 默认国家完成数量下限 10,000；省、县、街区单来源均衡采样上限 1,200/250/60，后台可覆盖；国家总量不是发布上限，行政区划与邮编目录保持完整。
...
- Google 土耳其层级映射：`administrative_area_level_4` 作为 district 的候选，按实际街道/门牌精度验证道路、行政层级、国家、种子几何与已知邮编；不把街道或普通地点名称当作行政区。
