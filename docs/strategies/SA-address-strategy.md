...
| 既有门牌级格式 / 邮编 | `<building/street>, <district>, <city> <postcode>, SA`；阿拉伯文原文保留；源记录；格式门禁 5 位或 `5-4` 扩展格式。只清洗格式，不创造或按邻近地址补齐 |
| 行政区 | 源字段与 catalog；GASTAT 行政目录作为候选核验。行政层级必须与坐标反查一致；冲突时保留源值在隔离区并拒绝发布 |
| 住宅子集证据 | OSM 住宅建筑标签；地理编码命中不等于住宅。住宅证据必须来自明确建筑/用途字段，不能由地址存在推断 |
| 发布门禁 | 接受真实街道级与已有门牌级地址；按实际精度检查来源、行政归属、语言和坐标。街道记录不强制门牌，不编造缺失字段；已有真实门牌保留，未知或歧义邮编留空；只有住宅子集要求独立住宅证据。 |
| 街道级格式 | `<street>, <district>, <city>, SA`；`matchLevel=street`，门牌、楼栋、室号为空；可唯一确定的来源邮编保留，否则留空。 |
...
| 既有门牌来源验证 / 排除 | 按 SAU 边界提取并核验真实道路、行政字段和坐标，避免邻国记录进入地址池；只有住宅子集要求精确关联明确住宅建筑面 |
| 策略版本 / 状态 / 更新 | 1.11 / 全国候选 + 严格住宅点面关联 + 多规则完成下限 + 街道契约 street-v1 / 2026-09-13 |

- 默认国家完成数量下限 5,000；地区、城市、区单来源均衡采样上限 1,000/200/50，后台可覆盖；国家总量不是发布上限，行政区划与邮编目录保持完整。
...
- 可选全国 OpenAddresses 归档使用 `archive-address-streets-v3`：保留来源道路、真实门牌和行政字段，未知邮编留空；住宅证据单独匹配。仅在 `ADDRESS_SYNC_SA_OPENADDRESSES_ENABLED` 及既有许可、再分发确认条件满足时启用；本轮没有启用或实测其容量。
